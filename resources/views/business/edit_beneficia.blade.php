...
<body class="bg-[#E9E9E9]  text-[#1E1E1E] min-h-screen flex flex-col md:flex-row">
  ...
  @include('business.header')
   ...
   @include('business.sidebar')
            ...
            <section
                ...
                <section class="flex flex-col md:flex-row gap-10 mx-auto max-w-full min-h-screen">
                    ...
                    <section class="flex-1 overflow-auto ">
                        ...
                        <table class="w-full border-collapse text-sm ">
                            ...
                            <tbody id="beneficiaries-table" class="divide-y divide-gray-200">
                                ...
                                <tr onclick="loadBeneficia(this)"
                                    data-id="{{ $b->id }}"
                                    data-country="{{ $b->country_id }}"
                                    data-bank="{{ $b->bank }}"
                                    data-account="{{ $b->account_number }}"
                                    data-name="{{ $b->account_name }}"
                    ...
                    <section class="w-full max-w-md border-l border-gray-200 pl-8">
                        ...
                        <form method="POST" id="edit-form" action="{{ route('beneficias.update', $beneficia->id) }}" class="flex flex-col gap-4 text-sm text-gray-600">
                            @csrf
                            @method('PUT')
                            @if ($errors->any())
                            <script>
                           ...
                           <!-- Bank -->
                            ...
                            <input type="text" id="account-number" name="account_number" value="{{ old('account_number', $beneficia->account_number) }}"
                                class="border border-gray-300 rounded-lg px-4 py-2 text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-300" />
                    
                            <!-- Account Name -->
                            <label for="account-name" class="font-normal">Bank account name</label>
                            <input type="text" id="account-name" name="account_name" value="{{ old('account_name', $beneficia->account_name) }}"
                                class="border border-gray-300 rounded-lg px-4 py-2 text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-300" />
    ...
    <script>
        // Function to load selected beneficiary data into the form
        function loadBeneficia(row) {
