...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const PAYAZA_CURRENCIES        = @json($payazaCurrencies);          // DB-driven
    const OHENTPAY_CURRENCIES      = @json($ohentpayCurrencies);        // DB-driven
    const XOF_COUNTRIES            = ["BJ", "BF", "CI", "GW", "ML", "NE", "SN", "TG"];
    const PROVIDER_CURRENCIES      = @json($providerCurrencies);
    ...
    const pivotEnabled     = @json($pivotEnabled);
    const payazaEnabled    = @json($payazaEnabled);
    const appmobileEnabled = @json($appmobileEnabled);
    const ohentpayEnabled  = @json($ohentpayEnabled);
    ...
    let ohentpayProviders = { bank: [], mobile: [] };
    ...
    // ← ADDED: OhentPay bank loader
    function loadOhentpayBanks() {
        const country  = countrySelect.value;
        const currency = normalizeCurrency(currencySelect.value);
        if (!country || !OHENTPAY_CURRENCIES.includes(currency) || !ohentpayEnabled) return;

        const url = new URL(banksFilterUrl, window.location.origin);
        url.searchParams.set("country", country);
        url.searchParams.set("currency", currency);
        url.searchParams.set("provider", "ohentpay");
        ...
        fetch(url.toString())
            ...
            .then(res => {
                ...
                ohentpayProviders = { bank: [], mobile: [] };
                list.forEach(i => {
                    if (i.type === "mobile_money") ohentpayProviders.mobile.push(i);
                    else ohentpayProviders.bank.push(i);
                });
                showTransferMethods();
            })
            .catch(err => console.error("OhentPay banks fetch error:", err));
    }
    ...
    function showTransferMethods() {
        ...
        // ← ADDED: OhentPay branch
        if (OHENTPAY_CURRENCIES.includes(currency) && ohentpayEnabled) {
            if (ohentpayProviders.bank.length)   html += '<option value="bank">Bank</option>';
            if (ohentpayProviders.mobile.length) html += '<option value="mobile">Mobile</option>';
            transferMethod.innerHTML = html;
            toggleField(transferMethod, ohentpayProviders.bank.length || ohentpayProviders.mobile.length, true);
            return;
        }
    ...
    function showFieldsByCurrency() {
        ...
        // ← ADDED: OhentPay checked before Payaza (matches backend priority)
        if (OHENTPAY_CURRENCIES.includes(currency) && ohentpayEnabled) { loadOhentpayBanks(); return; }
    ...
    function showFieldsByMethod() {
        ...
        if (OHENTPAY_CURRENCIES.includes(currency) && ohentpayEnabled) {
            if (method === "bank") {
                let html = '<option value="">Select Bank</option>';
                ohentpayProviders.bank.forEach(b => { html += `<option value="${b.bank_code || b.code}">${b.name}</option>`; });
                bankSelect.innerHTML = html;
            ...
            if (method === "mobile") {
                ...
                ohentpayProviders.mobile.forEach(p => { html += `<option value="${p.bank_code || p.code}">${p.name}</option>`; });
                bankSelect.innerHTML = html;
                toggleField(bankSelect, true, true);
    ...
    async function validateAccount() {
        ...
        if (currency === "UGX" && pivotEnabled) {
            ...
            } else if (OHENTPAY_CURRENCIES.includes(currency) && ohentpayEnabled) {
                // Only 3 inputs required — country is derived server-side from the bank row
        ...
        try {
            ...
            // console.log("Account inquiry response:", res.status, data);
