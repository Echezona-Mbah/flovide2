      ...
      <section class="bg-white text-gray-700 min-h-screen md:w-[80vw]   md:rounded-tl-3xl md:p-6 p-2 shadow-md md:absolute right-[-2vw] overflow-x-hidden ">
        <header class="flex items-center gap-4 mb-8">
          ...
          <div class="ml-auto flex items-center gap-4">
            <div class="flex items-center gap-2">
              <span class="w-3 h-3 rounded-full {{ $donation->visibility === "Private" ? "bg-yellow-500" : "bg-green-700" }} block"></span>
              <span class="text-gray-700 text-sm select-none">{{ $donation->visibility === "Private" ? "Draft" : "Active" }}</span>
            </div>
            <button type="button" class="md:flex hidden items-center gap-2 rounded-lg border {{ $donation->visibility === "Private" ? "border-gray-300 bg-gray-100 px-4 py-2 text-gray-500 cursor-not-allowed" : "border-blue-300 bg-blue-100 px-4 py-2 text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 hover:bg-blue-200" }} text-sm font-semibold">
        ...
        <main class="flex flex-col lg:flex-row gap-10">
          ...
          <section class="flex-1 max-w-full lg:max-w-[600px] flex flex-col gap-6">
            ...
            <form class="flex flex-col gap-6" action="{{ route('donation.update', ['id' => $donation->id]) }}" method="POST" enctype="multipart/form-data">
              @method('PUT')
              @csrf
              ...
              <div class="flex flex-col gap-1">
                ...
                <div id="cover-preview" class="mt-2 relative w-full h-28 border border-gray-300 rounded-md overflow-hidden {{ $donation->cover_image ? '' : 'hidden' }}">
                  @if($donation->cover_image)
                    <img id="cover-img" src="{{ asset('storage/'.$donation->cover_image) }}" alt="Cover Image" class="object-cover w-full h-full" />
                  @else
                ...
                {{-- Upload box --}}
                <label id="cover-upload-section" for="cover-upload" class="mt-2 cursor-pointer border border-dashed border-gray-300 rounded-md h-28 flex items-center justify-center text-xs text-gray-600 select-none hover:border-gray-400 {{ $donation->cover_image ? 'hidden' : '' }}">
              ...
              <div class="flex flex-col gap-1">
                ...
                <input id="title" name="title" type="text" value="{{ ucfirst($donation->title) }}" class="text-xs text-dark-700 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" />
              </div>

              <div class="flex flex-col gap-1">
                <label class="text-xs text-gray-600 select-none">Fixed Amount</label>
                <div class="grid grid-cols-1 items-center gap-3">
                  <input type="number" name="amount" value="{{ $donation->amount }}" class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" />
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="flex gap-3">
                  <select id="subaccount" name="subaccount_id" class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 flex-1 focus:outline-none focus:ring-1 focus:ring-blue-500">
                    <option value="" disabled {{ empty($donation->subaccount_id) ? 'selected' : '' }}> Select a subaccount </option>
                  ...
                  <input type="number" name="percentage" value="{{ $donation->percentage }}" class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 w-16 focus:outline-none focus:ring-1 focus:ring-blue-500" />
              ...
              <div class="flex flex-col gap-1">
                ...
                <select id="currency" name="currency" class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 flex items-center gap-2 focus:outline-none focus:ring-1 focus:ring-blue-500">
                  <option value="" disabled {{ empty($donation->currency) ? 'selected' : '' }}>🌍 Select currency</option>
                  ...
                  <option value="GBP" {{ $donation->currency === 'GBP' ? 'selected' : '' }}>🇬🇧 GBP – British Pound</option>
                  <option value="JPY" {{ $donation->currency === 'JPY' ? 'selected' : '' }}>🇯🇵 JPY – Japanese Yen</option>
                  <option value="CNY" {{ $donation->currency === 'CNY' ? 'selected' : '' }}>🇨🇳 CNY – Chinese Yuan</option>
                  <option value="INR" {{ $donation->currency === 'INR' ? 'selected' : '' }}>🇮🇳 INR – Indian Rupee</option>
                  <option value="AUD" {{ $donation->currency === 'AUD' ? 'selected' : '' }}>🇦🇺 AUD – Australian Dollar</option>
                  <option value="CAD" {{ $donation->currency === 'CAD' ? 'selected' : '' }}>🇨🇦 CAD – Canadian Dollar</option>
                  <option value="CHF" {{ $donation->currency === 'CHF' ? 'selected' : '' }}>🇨🇭 CHF – Swiss Franc</option>
                  <option value="NGN" {{ $donation->currency === 'NGN' ? 'selected' : '' }}>🇳🇬 NGN – Nigerian Naira</option>
                  <option value="ZAR" {{ $donation->currency === 'ZAR' ? 'selected' : '' }}>🇿🇦 ZAR – South African Rand</option>
                  <option value="KES" {{ $donation->currency === 'KES' ? 'selected' : '' }}>🇰🇪 KES – Kenyan Shilling</option>
                  <option value="GHS" {{ $donation->currency === 'GHS' ? 'selected' : '' }}>🇬🇭 GHS – Ghanaian Cedi</option>
                  <option value="EGP" {{ $donation->currency === 'EGP' ? 'selected' : '' }}>🇪🇬 EGP – Egyptian Pound</option>
                  <option value="BRL" {{ $donation->currency === 'BRL' ? 'selected' : '' }}>🇧🇷 BRL – Brazilian Real</option>
                  <option value="MXN" {{ $donation->currency === 'MXN' ? 'selected' : '' }}>🇲🇽 MXN – Mexican Peso</option>
                  <option value="ARS" {{ $donation->currency === 'ARS' ? 'selected' : '' }}>🇦🇷 ARS – Argentine Peso</option>
                  <option value="CLP" {{ $donation->currency === 'CLP' ? 'selected' : '' }}>🇨🇱 CLP – Chilean Peso</option>
                  <option value="COP" {{ $donation->currency === 'COP' ? 'selected' : '' }}>🇨🇴 COP – Colombian Peso</option>
                  <option value="PEN" {{ $donation->currency === 'PEN' ? 'selected' : '' }}>🇵🇪 PEN – Peruvian Sol</option>
                  <option value="RUB" {{ $donation->currency === 'RUB' ? 'selected' : '' }}>🇷🇺 RUB – Russian Ruble</option>
                  <option value="TRY" {{ $donation->currency === 'TRY' ? 'selected' : '' }}>🇹🇷 TRY – Turkish Lira</option>
                  <option value="SAR" {{ $donation->currency === 'SAR' ? 'selected' : '' }}>🇸🇦 SAR – Saudi Riyal</option>
                  <option value="AED" {{ $donation->currency === 'AED' ? 'selected' : '' }}>🇦🇪 AED – UAE Dirham</option>
                  <option value="QAR" {{ $donation->currency === 'QAR' ? 'selected' : '' }}>🇶🇦 QAR – Qatari Riyal</option>
                  <option value="KWD" {{ $donation->currency === 'KWD' ? 'selected' : '' }}>🇰🇼 KWD – Kuwaiti Dinar</option>
                  <option value="BHD" {{ $donation->currency === 'BHD' ? 'selected' : '' }}>🇧🇭 BHD – Bahraini Dinar</option>
                  <option value="OMR" {{ $donation->currency === 'OMR' ? 'selected' : '' }}>🇴🇲 OMR – Omani Rial</option>
                  <option value="PKR" {{ $donation->currency === 'PKR' ? 'selected' : '' }}>🇵🇰 PKR – Pakistani Rupee</option>
                  <option value="BDT" {{ $donation->currency === 'BDT' ? 'selected' : '' }}>🇧🇩 BDT – Bangladeshi Taka</option>
                  <option value="LKR" {{ $donation->currency === 'LKR' ? 'selected' : '' }}>🇱🇰 LKR – Sri Lankan Rupee</option>
                  <option value="THB" {{ $donation->currency === 'THB' ? 'selected' : '' }}>🇹🇭 THB – Thai Baht</option>
                  <option value="MYR" {{ $donation->currency === 'MYR' ? 'selected' : '' }}>🇲🇾 MYR – Malaysian Ringgit</option>
                  <option value="IDR" {{ $donation->currency === 'IDR' ? 'selected' : '' }}>🇮🇩 IDR – Indonesian Rupiah</option>
                  <option value="SGD" {{ $donation->currency === 'SGD' ? 'selected' : '' }}>🇸🇬 SGD – Singapore Dollar</option>
                  <option value="HKD" {{ $donation->currency === 'HKD' ? 'selected' : '' }}>🇭🇰 HKD – Hong Kong Dollar</option>
                  <option value="KRW" {{ $donation->currency === 'KRW' ? 'selected' : '' }}>🇰🇷 KRW – South Korean Won</option>
                  <option value="VND" {{ $donation->currency === 'VND' ? 'selected' : '' }}>🇻🇳 VND – Vietnamese Dong</option>
                  <option value="ILS" {{ $donation->currency === 'ILS' ? 'selected' : '' }}>🇮🇱 ILS – Israeli Shekel</option>
                  <option value="MAD" {{ $donation->currency === 'MAD' ? 'selected' : '' }}>🇲🇦 MAD – Moroccan Dirham</option>
                  <option value="TND" {{ $donation->currency === 'TND' ? 'selected' : '' }}>🇹🇳 TND – Tunisian Dinar</option>
                  <option value="DZD" {{ $donation->currency === 'DZD' ? 'selected' : '' }}>🇩🇿 DZD – Algerian Dinar</option>
                  <option value="ETB" {{ $donation->currency === 'ETB' ? 'selected' : '' }}>🇪🇹 ETB – Ethiopian Birr</option>
                  <option value="UGX" {{ $donation->currency === 'UGX' ? 'selected' : '' }}>🇺🇬 UGX – Ugandan Shilling</option>
                  <option value="TZS" {{ $donation->currency === 'TZS' ? 'selected' : '' }}>🇹🇿 TZS – Tanzanian Shilling</option>
                  <option value="RWF" {{ $donation->currency === 'RWF' ? 'selected' : '' }}>🇷🇼 RWF – Rwandan Franc</option>
                  <option value="XAF" {{ $donation->currency === 'XAF' ? 'selected' : '' }}>🌍 XAF – Central African CFA Franc</option>
                  <option value="XOF" {{ $donation->currency === 'XOF' ? 'selected' : '' }}>🌍 XOF – West African CFA Franc</option>
                  <option value="SCR" {{ $donation->currency === 'SCR' ? 'selected' : '' }}>🇸🇨 SCR – Seychellois Rupee</option>
                  <option value="MUR" {{ $donation->currency === 'MUR' ? 'selected' : '' }}>🇲🇺 MUR – Mauritian Rupee</option>
                  <option value="BWP" {{ $donation->currency === 'BWP' ? 'selected' : '' }}>🇧🇼 BWP – Botswana Pula</option>
                  <option value="NAD" {{ $donation->currency === 'NAD' ? 'selected' : '' }}>🇳🇦 NAD – Namibian Dollar</option>
                </select>
              </div>
              ...
              <div class="flex flex-col gap-1">
                ...
                <select id="visibility" name="visibility" class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500">
                  <option value="" disabled {{ empty($donation->visibility) ? "selected" : "" }}>Select visibility</option>
                  <option value="Public" {{ $donation->visibility === "Public" ? "selected" : "" }} >Public</option>
                  <option value="Private" {{ $donation->visibility === "Private" ? "selected" : "" }} >Private</option>
                </select>
              </div>
              ...
              <div class="flex gap-4 mt-6">
                ...
                <button type="button" onclick="deletedonation(this.value)" value="{{ $donation->id }}" class="text-red-600 bg-red-100 px-5 py-2 rounded-lg text-sm font-semibold hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-400">
                  Delete Page
                </button>
              </div>
            </form>
