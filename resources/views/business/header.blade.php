...
<div style="height:32px;"></div>
  ...
  <header class="bg-[#215F9C] p-4 flex items-center justify-between md:hidden">
    <!-- Sidebar toggle -->
    <button aria-label="Open sidebar" id="openSidebarBtn" class="text-white focus:outline-none">
      <i class="fas fa-bars text-2xl"></i>
    </button>

    <!-- User profile + greeting as clickable link -->
    <a href="{{ url('/organization_setting') }}" class="flex items-center gap-3">
      <img src="{{ auth()->user()->profile_picture ?? '../../asserts/dashboard/default-user.png' }}" alt="{{ auth()->user()->business_name }}" class="w-10 h-10 rounded-full object-cover border-2 border-gray-300" />
    ...
    <div class="flex items-center gap-2">
      <!-- Notifications -->
      <button id="notifyBtn_Mobile" aria-label="Notifications" class="relative w-10 h-10 flex items-center justify-center bg-white rounded-full">
      ...
      <form method="POST" action="{{ route('logout') }}" class="flex items-center m-0 p-0">
        @csrf
        <button type="submit" aria-label="Logout" class="w-10 h-10 flex items-center justify-center bg-red-500 rounded-full hover:bg-red-600 transition">
