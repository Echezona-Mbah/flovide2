      ...
      <section
        ...
        <header class="flex items-center gap-4 mb-8">
          ...
          <div class="ml-auto flex items-center gap-4">
            <div class="flex items-center gap-2">
              <span
                class="w-3 h-3 rounded-full {{ $remita->visibility === "Private" ? "bg-yellow-500" : "bg-green-700" }} block"></span>
              <span class="text-gray-700 text-sm select-none">
                {{ $remita->visibility === "Private" ? "Draft" : "Active" }}
              </span>
            </div>
            @if ($remita->visibility === "Private")
              <button type="button"
                class="flex items-center gap-2 rounded-lg border border-gray-300 bg-gray-100 px-4 py-2 text-gray-500 cursor-not-allowed text-sm font-semibold ">
                <i class="far fa-copy"></i>
                Copy Link
              </button>
            @else
              <button type="button" id="copyLinkBtn" onclick="copyToClipboard('{{ $remita->page_link }}')"
                class="md:flex hidden items-center gap-2 rounded-lg border border-blue-300 bg-blue-100 px-4 py-2 text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 hover:bg-blue-200 text-sm font-semibold">
                <i class="far fa-copy"></i>
                Copy Link
              </button>
            @endif
        ...
        <main class="flex flex-col lg:flex-row gap-10">
          ...
          <section class="flex-1 max-w-full lg:max-w-[600px] flex flex-col gap-6">
            ...
            <form class="flex flex-col gap-6" action="{{ route('remita.update', ['id' => $remita->id]) }}" method="POST"
              ...
              <div class="flex flex-col gap-1">
                ...
                <div id="cover-preview"
                  class="mt-2 relative w-full h-28 border border-gray-300 rounded-md overflow-hidden {{ $remita->cover_image ? '' : 'hidden' }}">
                  @if($remita->cover_image)
                    <img id="cover-img" src="{{ asset('storage/' . $remita->cover_image) }}" alt="Cover Image"
                ...
                <label id="cover-upload-section" for="cover-upload"
                  class="mt-2 cursor-pointer border border-dashed border-gray-300 rounded-md h-28 flex items-center justify-center text-xs text-gray-600 select-none hover:border-gray-400 {{ $remita->cover_image ? 'hidden' : '' }}">
              ...
              <div class="flex flex-col gap-1">
                ...
                <input id="title" name="title" type="text" value="{{ ucfirst($remita->title) }}"
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="grid grid-cols-1 items-center gap-3">
                  <input type="number" name="amount" value="{{ $remita->amount }}"
              ...
              <div class="flex flex-col gap-1">
                ...
                <select id="service-type" name="service_type"
                  class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500">
                  <option value="" disabled {{ empty($remita->service_type) ? 'selected' : '' }}> Select a service type
                  </option>
                  <option value="school_fees" {{ $remita->service_type === 'school_fees' ? 'selected' : '' }}>School Fees
                  </option>
                  <option value="utilities" {{ $remita->service_type === 'utilities' ? 'selected' : '' }}>Utilities
                  </option>
                  <option value="donation" {{ $remita->service_type === 'donation' ? 'selected' : '' }}>Donation</option>
                  <option value="subscription" {{ $remita->service_type === 'subscription' ? 'selected' : '' }}>
                    Subscription</option>
                  <option value="product_payment" {{ $remita->service_type === 'product_payment' ? 'selected' : '' }}>
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="flex gap-3">
                  <select id="subaccount" name="subaccount_id"
                    ...
                    <option value="" disabled {{ empty($remita->subaccount_id) ? 'selected' : '' }}> Select a subaccount
                    </option>
                  ...
                  <input type="number" name="percentage" value="{{ $remita->percentage }}"
              ...
              <div class="flex flex-col gap-1">
                ...
                <select id="currency" name="currency"
                  ...
                  <option value="GBP" {{ $remita->currency === 'GBP' ? 'selected' : '' }}>🇬🇧 GBP – British Pound
                  </option>
                  ...
                  <option value="AUD" {{ $remita->currency === 'AUD' ? 'selected' : '' }}>🇦🇺 AUD – Australian Dollar
                  </option>
                  <option value="CAD" {{ $remita->currency === 'CAD' ? 'selected' : '' }}>🇨🇦 CAD – Canadian Dollar
                  </option>
                  <option value="CHF" {{ $remita->currency === 'CHF' ? 'selected' : '' }}>🇨🇭 CHF – Swiss Franc</option>
                  <option value="NGN" {{ $remita->currency === 'NGN' ? 'selected' : '' }}>🇳🇬 NGN – Nigerian Naira
                  </option>
                  <option value="ZAR" {{ $remita->currency === 'ZAR' ? 'selected' : '' }}>🇿🇦 ZAR – South African Rand
                  </option>
                  <option value="KES" {{ $remita->currency === 'KES' ? 'selected' : '' }}>🇰🇪 KES – Kenyan Shilling
                  </option>
                  <option value="GHS" {{ $remita->currency === 'GHS' ? 'selected' : '' }}>🇬🇭 GHS – Ghanaian Cedi
                  </option>
                  <option value="EGP" {{ $remita->currency === 'EGP' ? 'selected' : '' }}>🇪🇬 EGP – Egyptian Pound
                  </option>
                  <option value="BRL" {{ $remita->currency === 'BRL' ? 'selected' : '' }}>🇧🇷 BRL – Brazilian Real
                  </option>
                  <option value="MXN" {{ $remita->currency === 'MXN' ? 'selected' : '' }}>🇲🇽 MXN – Mexican Peso</option>
                  <option value="ARS" {{ $remita->currency === 'ARS' ? 'selected' : '' }}>🇦🇷 ARS – Argentine Peso
                  </option>
                  <option value="CLP" {{ $remita->currency === 'CLP' ? 'selected' : '' }}>🇨🇱 CLP – Chilean Peso</option>
                  <option value="COP" {{ $remita->currency === 'COP' ? 'selected' : '' }}>🇨🇴 COP – Colombian Peso
                  </option>
                  <option value="PEN" {{ $remita->currency === 'PEN' ? 'selected' : '' }}>🇵🇪 PEN – Peruvian Sol</option>
                  <option value="RUB" {{ $remita->currency === 'RUB' ? 'selected' : '' }}>🇷🇺 RUB – Russian Ruble
                  </option>
                  ...
                  <option value="KWD" {{ $remita->currency === 'KWD' ? 'selected' : '' }}>🇰🇼 KWD – Kuwaiti Dinar
                  </option>
                  <option value="BHD" {{ $remita->currency === 'BHD' ? 'selected' : '' }}>🇧🇭 BHD – Bahraini Dinar
                  </option>
                  <option value="OMR" {{ $remita->currency === 'OMR' ? 'selected' : '' }}>🇴🇲 OMR – Omani Rial</option>
                  <option value="PKR" {{ $remita->currency === 'PKR' ? 'selected' : '' }}>🇵🇰 PKR – Pakistani Rupee
                  </option>
                  <option value="BDT" {{ $remita->currency === 'BDT' ? 'selected' : '' }}>🇧🇩 BDT – Bangladeshi Taka
                  </option>
                  <option value="LKR" {{ $remita->currency === 'LKR' ? 'selected' : '' }}>🇱🇰 LKR – Sri Lankan Rupee
                  </option>
                  <option value="THB" {{ $remita->currency === 'THB' ? 'selected' : '' }}>🇹🇭 THB – Thai Baht</option>
                  <option value="MYR" {{ $remita->currency === 'MYR' ? 'selected' : '' }}>🇲🇾 MYR – Malaysian Ringgit
                  </option>
                  <option value="IDR" {{ $remita->currency === 'IDR' ? 'selected' : '' }}>🇮🇩 IDR – Indonesian Rupiah
                  </option>
                  <option value="SGD" {{ $remita->currency === 'SGD' ? 'selected' : '' }}>🇸🇬 SGD – Singapore Dollar
                  </option>
                  <option value="HKD" {{ $remita->currency === 'HKD' ? 'selected' : '' }}>🇭🇰 HKD – Hong Kong Dollar
                  </option>
                  <option value="KRW" {{ $remita->currency === 'KRW' ? 'selected' : '' }}>🇰🇷 KRW – South Korean Won
                  </option>
                  <option value="VND" {{ $remita->currency === 'VND' ? 'selected' : '' }}>🇻🇳 VND – Vietnamese Dong
                  </option>
                  <option value="ILS" {{ $remita->currency === 'ILS' ? 'selected' : '' }}>🇮🇱 ILS – Israeli Shekel
                  </option>
                  <option value="MAD" {{ $remita->currency === 'MAD' ? 'selected' : '' }}>🇲🇦 MAD – Moroccan Dirham
                  </option>
                  <option value="TND" {{ $remita->currency === 'TND' ? 'selected' : '' }}>🇹🇳 TND – Tunisian Dinar
                  </option>
                  <option value="DZD" {{ $remita->currency === 'DZD' ? 'selected' : '' }}>🇩🇿 DZD – Algerian Dinar
                  </option>
                  <option value="ETB" {{ $remita->currency === 'ETB' ? 'selected' : '' }}>🇪🇹 ETB – Ethiopian Birr
                  </option>
                  <option value="UGX" {{ $remita->currency === 'UGX' ? 'selected' : '' }}>🇺🇬 UGX – Ugandan Shilling
                  </option>
                  <option value="TZS" {{ $remita->currency === 'TZS' ? 'selected' : '' }}>🇹🇿 TZS – Tanzanian Shilling
                  </option>
                  <option value="RWF" {{ $remita->currency === 'RWF' ? 'selected' : '' }}>🇷🇼 RWF – Rwandan Franc
                  </option>
                  <option value="XAF" {{ $remita->currency === 'XAF' ? 'selected' : '' }}>🌍 XAF – Central African CFA
                    Franc</option>
                  <option value="XOF" {{ $remita->currency === 'XOF' ? 'selected' : '' }}>🌍 XOF – West African CFA Franc
                  </option>
                  <option value="SCR" {{ $remita->currency === 'SCR' ? 'selected' : '' }}>🇸🇨 SCR – Seychellois Rupee
                  </option>
                  <option value="MUR" {{ $remita->currency === 'MUR' ? 'selected' : '' }}>🇲🇺 MUR – Mauritian Rupee
                  </option>
                  <option value="BWP" {{ $remita->currency === 'BWP' ? 'selected' : '' }}>🇧🇼 BWP – Botswana Pula
                  </option>
                  <option value="NAD" {{ $remita->currency === 'NAD' ? 'selected' : '' }}>🇳🇦 NAD – Namibian Dollar
                  </option>
                </select>
              </div>
              ...
              <div class="flex flex-col gap-1">
                ...
                <select id="visibility" name="visibility"
                  class="text-xs text-gray-700 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500">
                  <option value="" disabled {{ empty($remita->visibility) ? "selected" : "" }}>Select visibility
                  </option>
              ...
              <div class="flex gap-4 mt-6">
                ...
                <button type="button" onclick="deleteRemita(this.value)" value="{{ $remita->id }}"
                  ...
                  <div class="flex items-center gap-4 bg-white justify-between rounded-xl px-6 py-3 shadow-sm">
                    <span class="bg-[#F3F3F3] p-2 rounded-md">
                      @if ($payment->status === 'pending')
                        <span class="w-3 h-3 rounded-full block bg-orange-500" aria-label="Pending status"></span>
                      @elseif ($payment->status === 'failed')
                        <span class="w-3 h-3 rounded-full block bg-red-600" aria-label="Failed status"></span>
                      @elseif ($payment->status === 'successful')
                        <span class="w-3 h-3 rounded-full block bg-green-700" aria-label="Success status"></span>
                      @else
                        <span class="w-3 h-3 rounded-full block bg-gray-500" aria-label="Unknown status"></span>
                      @endif
                    </span>

                    <span class="font-semibold text-gray-900 text-[10px]">{{ $payment->name }}</span>
                    <span
                      class="bg-gray-100 text-gray-600 text-[10px] font-semibold rounded-full px-2 py-1">{{ $payment->email }}</span>
                    <span
                      class="bg-gray-100 text-gray-600 text-[10px] font-semibold rounded-full px-3 py-1">{{ $payment->number }}</span>
                  </div>
                @endforeach
              @endif
            </div>
