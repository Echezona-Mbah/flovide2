...
<body>
                                ...
                                <div class="page-title-icon">
                                    ...
                                    <div>
                                        ...
                                        <div class="page-title-subheading">
                                            View and manage all your transactions, including status, amounts, and payment methods. 
                                            Use the search box to quickly find specific transactions.
                                        </div>
                                    </div>
                            </div>
   ...
   {{-- <style>
     td {
        ...
        max-width: 180px; /* Optional: control width */
    }
</style> --}}
                    ...
                    <div class="main-card mb-3 card">
                            <div class="card-header">
                                <div class="card-header-title font-size-lg text-capitalize fw-normal">All Transaction History
                                </div>
                            </div>
                            <div class="mb-3">
                                ...
                                <tbody>
                                    @foreach ($lastTransactions as $item)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $item->reference ?? 'N/A' }}</td>
                                            <td class="text-center">
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <button type="button" class="btn btn-primary view-transaction-btn"
                                                        ...
                                                        <button type="button" 
                                                                class="btn btn-warning chargeback-btn" 
                                                                ...
                                                                data-reference="{{ $item->reference }}"
                                                                data-amount="{{ number_format($item->amount, 2) }}"
                                                                data-currency="{{ $item->currency }}"> <!-- Pass currency -->
                                                            Chargeback
                                                        </button>
  ...
  <div class="modal-dialog modal-dialog-centered">
    ...
    <form method="POST" action="{{ route('admin.chargeback.submitEvidence') }}" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            ...
            <input type="hidden" name="user_id" id="cbUserId"> <!-- hidden user_id -->
            <input type="hidden" name="currency" id="cbCurrency"> <!-- hidden currency -->
            ...
            <div class="mb-3">
                <label class="form-label">Reasone</label>
                <textarea name="reason" class="form-control" rows="3" placeholder="Explain reason for chargeback"></textarea>
            </div>
        </div>

        <div class="modal-footer">
            <button type="submit" class="btn btn-primary">Submit Chargeback</button>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chargebackButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const userId = btn.getAttribute('data-user-id'); // get user_id
            const reference = btn.getAttribute('data-reference');
            const amount = btn.getAttribute('data-amount');
            const currency = btn.getAttribute('data-currency');

            document.getElementById('cbTransactionId').value = transactionId;
            document.getElementById('cbUserId').value = userId;      // fill user_id
            document.getElementById('cbCurrency').value = currency;  // fill currency
            document.getElementById('cbReference').value = reference;
            document.getElementById('cbAmount').value = `${amount} ${currency}`;
        });
    });
});
</script>
