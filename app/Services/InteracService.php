...
class InteracService
{
    ...
    public function __construct(protected InteracIamTokenService $tokenService)
    {
    ...
    protected function request(
        ...
        try {
            ...
            $send = function (string $token) use ($method, $url, $payload) {
                $options = [];
                if (!in_array($method, ['GET', 'DELETE'], true) && !is_null($payload)) {
            ...
            // retry once if unauthorized (force refresh token)
    ...
    {
        $providerCode = is_array($json) ? ($json['error'][0]['code'] ?? null) : null;
        $additional = is_array($json) ? ($json['error'][0]['additional_information'] ?? null) : null;
