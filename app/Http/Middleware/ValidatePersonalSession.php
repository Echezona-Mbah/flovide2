...
class ValidatePersonalSession
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $tokenSessionId = substr(
            $sessionAbility,
            strlen('session:')
        );

        // Compare token session with the currently active account session
        if (
            ...
            // Remove the stale token after detecting it
            $token->delete();
