...
public function create()
{
    ...
    // ── Provider/currency state now comes from the DB, not .env ───────────
    $routing = app(\App\Services\ProviderRoutingService::class);

    $pivotEnabled     = $routing->isEnabled('pivot');
    $payazaEnabled    = $routing->isEnabled('payaza');
    $appmobileEnabled = $routing->isEnabled('app_mobile');
    $ohentpayEnabled  = $routing->isEnabled('ohentpay');

    $payazaCurrencies    = $routing->enabledCurrenciesFor('payaza');
    $pivotCurrencies     = $routing->enabledCurrenciesFor('pivot');
    $appmobileCurrencies = $routing->enabledCurrenciesFor('app_mobile');
    $ohentpayCurrencies  = $routing->enabledCurrenciesFor('ohentpay');

    $providerCurrencies = array_values(array_unique(array_merge(
        $payazaCurrencies,
        $pivotCurrencies,
        $appmobileCurrencies,
        $ohentpayCurrencies
...
public function store(Request $request)
{
    ...
    if (!$resolvedProvider) {
        Log::warning('[Beneficiary Store] No provider enabled for currency', ['currency' => $currency, 'method' => $method]);
        return $isApi
    ...
    if ($provider === 'payaza' && $currency === 'UGX') {
        ...
        if ($provider === 'ohentpay') {
            ...
            Log::info('[Beneficiary Store] OhentPay bank resolution', [
            ...
            $recipientPayload = [
                ...
                'bank_id'        => $bankRow->provider_bank_id,   // ← the actual field OhentPay expects
            ...
            if (!$ohentPayResponse['success']) {
                Log::error('[Beneficiary Store] OhentPay recipient creation failed', [
        ...
        try {
            $beneficia = Beneficia::create([
                ...
                'ohentpay_recipient_id' => $ohentPayRecipientId ?? null,
